...
import poscs.common.CustomerEvaluator;
import poscs.dao.AddressDAO;
import poscs.dao.ContractDAO;
import poscs.dao.ContractPaymentDAO;
import poscs.dao.CustomerDAO;
import poscs.dao.CustomerEvaluationRuleDAO;
...
public class CustomerController extends HttpServlet {
    ...
    private final CustomerEvaluationRuleDAO evaluationRuleDAO = new CustomerEvaluationRuleDAO();
    ...
    /**
     * Chạy CustomerEvaluator trên lịch sử thanh toán hiện tại của khách hàng,
     * ghi kết quả vào enterprises.current_relationship_rating và thêm 1 dòng
     * customer_lifecycle_events (is_auto_generated=1) làm bằng chứng/lịch sử
     * cho lần đánh giá này -- cùng quyền Full access với create/update/delete
     * vì đây cũng là 1 thao tác ghi dữ liệu khách hàng.
     */
    private void handleEvaluate(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        List<ContractPayment> payments = paymentDAO.findByEnterpriseId(id);
        Map<String, BigDecimal> rules = evaluationRuleDAO.findAllAsMap();
        CustomerEvaluator.Result result = CustomerEvaluator.evaluate(payments, rules);

        customerDAO.updateRelationshipRating(id, result.getRating());
        ...
        event.setRelationshipRating(result.getRating());
        event.setAutoGenerated(true);
        event.setDescription(result.getReason());
